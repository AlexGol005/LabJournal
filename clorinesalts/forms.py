...
class StrJournalCreationForm(forms.ModelForm):
    ...
    constit = forms.ChoiceField(label='Диапазон хлористых солей, мг/л', required=True,
                             choices=CHOICES,
                             widget=forms.Select(attrs={'class': 'form-control'}))
    projectconc = forms.CharField(label='Расчётное содержание хлористых солей, мг/л)', max_length=90,
                              widget=forms.TextInput(attrs={'class': 'form-control',
                                                            'placeholder': '000.00'}))
    ...
    solvent = forms.ChoiceField(label='Растворитель', required=True,
                             choices=SOLVENTS,
                             widget=forms.Select(attrs={'class': 'form-control'}))
    truevolume = forms.BooleanField(label='Для каждой экстракции: горячей воды на экстракцию 100 мл, промывка  + 35 мл, промывка фильтра + 15 мл',
                                    required=True)
